...
const graphPath = path.join(__dirname, 'synthetic_graph.json');
const graph = JSON.parse(fs.readFileSync(graphPath, 'utf8'));

console.log("=== PHASE 4A PROOF SCRIPT ===");

// 1. Graph load & Nodes
assert(graph.controlledJunctions.length === 6, "TEST 1 FAILED: Graph junctions not loaded");
const jNames = new Set(graph.controlledJunctions.map(j => j.name));
...
assert(Math.abs(res.individual.estimatedTime - 1030) < 5, "TEST 4 FAILED: Base travel time mismatch");
...
assert(resModerate.individual.estimatedTime > res.individual.estimatedTime, "TEST 5 FAILED: Individual didn't incur congestion");
...
assert(resModerate.aura.estimatedTime > 0, "TEST 6 FAILED: AURA ETA is invalid");
assert(resModerate.aura.explanation.includes("saturated Palarivattom (46%)"), "TEST 6 FAILED: Explanation missing");
...
assert(resExtreme.individual.route.join('->') === "J1->J5->J6->J3" || resExtreme.individual.route.length > 0, "TEST 7 FAILED: Individual didn't divert on extreme congestion");
assert(resExtreme.individual.estimatedTime > 0, "TEST 7 FAILED: Cost mismatch");
